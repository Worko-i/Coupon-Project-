...
class CustomerCouponService {
    ...
    async purchaseCoupon(couponId: number): Promise<void> {
    ...
    async getCouponsByCustomer(): Promise<CouponModel[]> {
        // in order to prevent an error, logout if the token expired or null
        if(tokenService.TokenExpiredHandler(authStore.getState()?.token!)){
            return [];
        }
        ...
        const response = await axios.get<CouponModel[]>(
            appConfig.apiAddress + `company/customer_coupon/byCustomer/${customerId}`,
            {headers: {"Authorization" : "Bearer " + authStore.getState().token}}
        );
